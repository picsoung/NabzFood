	...
	if($form_signup->is_valid($_POST)) {
		...
		if(empty($error_signup)) {
			...
			if (ctype_digit($id_user)) {
				...
				// Preparation du mail
				$message_mail = "<html><head></head><body> <p>Merci de vous être inscrit sur <b>Nabzfood</b> !</p>
				<p>Veuillez cliquer sur <a href=\"http://".$_SERVER['PHP_SELF']."?module=members&amp;action=valid_account&amp;hash=".$hash_validation."\">ce lien</a> pour activer votre compte !</p></body></html>";
				
				$headers_mail  = 'MIME-Version: 1.0'                           ."\r\n";
				$headers_mail .= 'Content-type: text/html; charset=utf-8'      ."\r\n";
				$headers_mail .= 'From: "Mon site" <contact@monsite.com>'      ."\r\n";
				
				$email_addr = $form_signup->get_cleaned_data('email_addr');
				// Envoi du mail
