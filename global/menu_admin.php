<?php
	function display_menu_admin(){
		...
		$id = '&id='.$_SESSION['id'];
	$tab_menu_link = array( $url.'add_productandcat'.$id, $url.'add_quantity'.$id, $url.'manage_product'.$id, $url.'users'.$id,$url.'nabz'.$id,$url.'manage_bank'.$id);
	$tab_menu_text = array( "Ajouter un(e) produit/catégorie", "Stocks", "Produits", "Utilisateurs","Lapins","Banque");
